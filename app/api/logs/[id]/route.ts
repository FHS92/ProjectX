...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const {
    ...
    visibility, waterTemp, gasMix, tankStart, tankEnd, weightUsed, diveType, buddy,
    projectName, materialUsed,
    shotgunDiscipline, score, roundsTotal, dtlFirstBarrel, dtlSecondBarrel,
  ...
  const [entry] = await db.update(logs).set({
    ...
    gasMix:              gasMix || null,
    tankStart:           tankStart ? parseInt(tankStart) : null,
    tankEnd:             tankEnd ? parseInt(tankEnd) : null,
    ...
    dtlFirstBarrel:      dtlFirstBarrel ? parseInt(dtlFirstBarrel) : null,
    dtlSecondBarrel:     dtlSecondBarrel ? parseInt(dtlSecondBarrel) : null,
